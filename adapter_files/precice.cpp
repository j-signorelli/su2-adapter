      ...
      if (readDataType != ReadDataType::Temperature) {
        ...
        for (int iVertex = 0; iVertex < vertexSize[i]; iVertex++) {
          ...
          if (geometry_container[ZONE_0][INST_0][MESH_0]->nodes->GetColor(nodeVertex[iVertex]) == solverProcessIndex) {
            heatFluxes[iVertex] =
                factor * solver_container[ZONE_0][INST_0][MESH_0][heat_sol]->GetHeatFlux(valueMarkerWet[i], iVertex) * -1;
            cout << "Heat flux retrieved before sending to precice for vertex " << iVertex << ": " << heatFluxes[iVertex] << endl;
          } else {
