...
class Precice {
 ...
 public:
   ...
   *
   * \param[in] action - Reference to the strings coric or cowic, which determine wether read (coric) or write (cowic)
   * action is checked. \return True if an action is required or false if no action is required.
   */
  bool isActionRequired(const string& action);

  /*!
   * \brief Get reference to the string cowic which determines wether an iteration checkpoint needs to be written or
   * not.
   *
   ...
   *
   * \param[in] StopCalc - Pointer to the bool StopCalc, which determines wether SU2 finishes computation after the
   * current run or not. This variable must be passed so that its value can be stored and reloaded later. \param[in]
   * double - Pointer to the double dt, which is the current SU2 time step size. This variable must be passed so that
   * its value can be stored and reloaded later.
   */
   ...
   *
   * \param[in] StopCalc - Pointer to the bool StopCalc, which determines wether SU2 finishes computation after the
   * current run or not. This variable must be passed so that its value can be reloaded from the saved state. \param[in]
   * double - Pointer to the double dt, which is the current time step size. This variable must be passed so that its
   * value can be reloaded from the saved state.
   */
